...
describe("K_nine Collection", function () {
  ...
  it("Should create the  correct tokenURI", async function () {
    ...
    let format_data = uri => JSON.parse(atob(uri.split('base64,')[1]).replaceAll("'", '"'));
  ...
  it("K_9 revive price should be 4 celo", async function () {
    ...
    expect((await altdog.revive_price()).toString()).to.equal("4000000000000000000")
  ...
  it("K_9 revive method works as expected", async function () {
    ...
    const tx2 = await altdog.connect(acc2).revive(0, { value: BigNumber.from("4000000000000000000") })
    await tx2.wait();
