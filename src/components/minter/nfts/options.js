  ...
  return !isDead ? (
    <>
      <Card.Text className="flex-grow-1">
        <div className="mb-3">
          <ProgressBar style={{ position: "relative" }}>
            <ProgressBar
              variant={fedPercent > 51 ? "success" : "danger"}
              now={fedPercent}
              style={{ minWidth: 10 }}
            />
        ...
        <div>
          <ProgressBar style={{ position: "relative" }}>
            <ProgressBar
              variant={petPercent > 51 ? "success" : "danger"}
              now={petPercent}
              style={{ minWidth: 10 }}
            />
      ...
      <div>
        <Row className="mt-2">
          <Col key={"first"}>
            <Button onClick={() => feed(index)} variant="primary">
              Feed
            </Button>{" "}
          </Col>
          <Col key={"second"}>
            <Button onClick={() => pet(index)} variant="primary">
              Pet
            </Button>{" "}
          </Col>
        </Row>
    ...
    <>
      <Card.Text className="flex-grow-1">
        ...
        <div>
          <img
            src={skullImg}
            alt={"dead"}
            style={{ width: 32, height: "auto", color: "red" }}
          />
        </div>
        {/* </Stack> */}
      </Card.Text>
      <div>
        <Row className="mt-2">
          <Col key={"test"}>
            <Button onClick={() => revive(index)} variant="danger">
              Revive
            </Button>
          </Col>
        </Row>
